  ...
  try {
    ...
    results.forEach((result) => {
      const timeDiff: number =
        Number(new Date(result.loans.expiresOn)) -
        Number(new Date(result.loans.loanedAt));
      ...
      const month = String(result.loans.expiresOn.getMonth() + 1).padStart(
        2,
        "0"
      ...
      const authorsSet: Set<string> =
        booksWithAuthors[Number(result.books?.bookId)];
...
export function getCheckoutDateNum(currentDayNum: number) {
  if (currentDayNum >= 0 && currentDayNum <= 4) return 1;
