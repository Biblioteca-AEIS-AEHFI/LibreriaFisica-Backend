...
export class UserHome {
  static async getRecommendedBooksByMostUserLoans(numeroCuenta: string) {
    ...
    while (
      ...
      const authorsNames: any = formatAuthorsNames(
        booksWithMostLoansByCategory
      );
      booksWithMostLoansByCategory.forEach((res) => {
        ...
        const obj = {
          authors: formattedAuthorsNames,
          title: res.books.title,
          bookId: res.books.bookId,
          isbn: res.books.isbn,
        };
        if (
          recommendedBooksByMostUserLoans.filter(
            (book) => book.bookId == obj.bookId
          ).length == 0
        )
  ...
  static async getPopularBooks() {
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        const authorsNamesSet: Set<string> =
          authorsNames[Number(results[i].books?.bookId)];
        const authorsNamesFormatted = getAuthorsNames(authorsNamesSet);
        const bookObj = {
          authors: authorsNamesFormatted,
          bookId: results[i].books?.bookId,
          title: results[i].books?.title,
          isbn: results[i].books?.isbn,
        };
        if (
          booksMostReserved.filter((book) => book.bookId == bookObj.bookId)
            .length == 0
        )
  ...
  static async getLastAdded() {
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        const authorsNamesSet: Set<string> =
          authorsNamesObj[results[i].books.bookId];
        ...
        if (
          lastAddedBooks.filter((book) => book.bookId == bookObj.bookId)
            .length == 0
        )
